...
object DisplayPerfSampler {
    ...
    fun setActive(activity: Activity?, enabled: Boolean) {
        activity?.runOnUiThread {
            ...
            samples = 0
            if (enabled) {
                refreshRate = activity.display?.refreshRate?.toDouble()?.takeIf { it > 0 } ?: 60.0
    ...
    @Synchronized fun appendTo(out: JSONObject) {
        ...
        val elapsed = if (windowStartNs > 0) (now - windowStartNs) / 1_000_000_000.0 else 0.0
        val fps = (if (elapsed > 0) frames / elapsed else 0.0).coerceAtMost(refreshRate)
        val avgMs = if (frames > 0) totalDeltaNs / 1_000_000.0 / frames else 0.0
        out.put("frameTimeMs", avgMs)
            .put("rollingFrameTimeMs", avgMs)
            .put("fps", fps)
            .put("targetRefreshRate", refreshRate)
            .put("droppedFrames", dropped)
            .put("jankyFrames", janky)
            .put("sampleFrames", samples)
        windowStartNs = now
        frames = 0
        totalDeltaNs = 0
    }
