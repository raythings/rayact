...
object DevClientBridge {
    ...
    fun handle(method: String, data: JSONObject?): Any? {
        return when (method) {
            ...
            "setPerformanceSampling" -> { DisplayPerfSampler.setActive(liveActivity(), data?.optBoolean("active") == true); null }
    ...
    private fun getAppInfo(): String {
        ...
        return JSONObject()
            ...
            .put("nativeAppVersion", BuildConfig.VERSION_NAME ?: "0.0.5")
            .put("rayactVersion", "0.0.5")
            .toString()
    }
    ...
    private fun getPerformanceMetrics(): String {
        ...
        val cpu = if (ticks == null || lastCpuTicks < 0 || lastCpuWallMs < 0) 0.0 else {
        ...
        lastCpuWallMs = now
        return JSONObject().put("cpuPercent", cpu)
            .put("memoryMb", Debug.getPss().toDouble() / 1024.0)
            .also(DisplayPerfSampler::appendTo).toString()
    }
