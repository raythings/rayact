...
internal class RayactDevToolsTransport(
    ...
    fun stop() {
        active = false; cdpSessionId = null
        socket?.close(1000, "Rayact session stopped"); socket = null
    ...
    @Synchronized private fun reconnect(failed: WebSocket) {
        ...
        socket = null; cdpSessionId = null
        if (!active || !session.isAlive()) return
        val delay = retryMs; retryMs = (retryMs * 2).coerceAtMost(5_000L)
