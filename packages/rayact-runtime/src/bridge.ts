import type { HostBridge, HostEventName, HostNode, HostNodeType, RayactGlobal } from './types.js';
import { isRayactAsset, resolveAssetUrl } from './assets.js';
...
function toStyleProps(props: Record<string, unknown> = {}, isCreate: boolean, nodeId?: number): Record<string, unknown> {
  ...
  for (const key of [
    ...
    'focusable', 'tabIndex', 'accessibilityLiveRegion', 'allowFontScaling',
...
export function createBridge(globalObject: RayactGlobal = globalThis as RayactGlobal): HostBridge {
  ...
  const bridge: HostBridge = {
    createNode(type, props = {}) {
      ...
      switch (type) {
        ...
        case 'view': {
          ...
          const viewArg: Record<string, unknown> =
            props.appBarTitle ? { ...style, appBarTitle: true } : { ...style };
          // Not a style key — forwarded so plain Views can keep the IME open
          // when tapped (RN keyboardShouldPersistTaps).
          if (typeof props.keyboardShouldPersistTaps === 'string')
            viewArg.keyboardShouldPersistTaps = props.keyboardShouldPersistTaps;
          // Keep semantic ids available to native diagnostics/inspection.
          // They are metadata, not visual style, so forward them explicitly.
          if (typeof props.id === 'string') viewArg.id = props.id;
    ...
    updateNode(node, props) {
      ...
      if (
        node.type === 'textInput' &&
        typeof native.setTextInputProps === 'function' &&
        (
          'multiline' in props ||
          'blurOnSubmit' in props ||
          'readOnly' in props ||
          'disabled' in props ||
          'inputType' in props ||
          'imeAction' in props ||
          'autocorrect' in props ||
    ...
    showError(message, stack) {
      ...
      const devCall = typeof native.devCall === 'function' ? native.devCall.bind(native) : null;
      const canReload = !!devCall || typeof globalThis.location?.reload === 'function';
      const canShowLauncher = !!devCall;
      ...
      const callDevClient = (method: string) => {
        if (devCall) devCall(method, undefined, () => {});
      };
      try {
        const root = bridge.createNode('safeArea', {
          style: {
            backgroundColor: 0x2B1111FF,
            flexGrow: 1
          }
        });
        const scroller = bridge.createNode('scrollView', {
          style: {
            flexGrow: 1,
            padding: 24
          }
        });
        const content = bridge.createNode('view', {
          style: {
            gap: 12,
            flexGrow: 1
          }
        ...
        const status = bridge.createNode('text', {
          text: 'The project failed while running. Fix the error in the dev server, then reload the project.',
          style: { text: { color: 0xFFD7D7D7FF, fontSize: 14 } }
        });
        const body = bridge.createNode('text', {
          text: detail,
          style: { text: { color: 0xFFB4B4FF, fontSize: 14 } }
        });
        bridge.appendChild(content, title);
        bridge.appendChild(content, status);
        if (canReload || canShowLauncher || canCopy) {
          const actions = bridge.createNode('view', {
            style: {
              flexDirection: 'row',
              flexWrap: 'wrap',
              gap: 8
          ...
          if (canReload) {
            const reload = bridge.createNode('button', { label: 'Reload' });
            bridge.setEventHandler(reload, 'press', () => {
              if (devCall) {
                callDevClient('reloadWithProjectBundle');
              } else {
                globalThis.location?.reload();
              }
            });
            bridge.appendChild(actions, reload);
          }
          if (canShowLauncher) {
            const launcher = bridge.createNode('button', { label: 'Return to launcher' });
            bridge.setEventHandler(launcher, 'press', () => callDevClient('showLauncher'));
            bridge.appendChild(actions, launcher);
          }
          if (canCopy) {
            const copy = bridge.createNode('button', { label: 'Copy details' });
            bridge.setEventHandler(copy, 'press', () => { void clipboard.writeText?.(detail); });
            bridge.appendChild(actions, copy);
          }
          bridge.appendChild(content, actions);
        }
