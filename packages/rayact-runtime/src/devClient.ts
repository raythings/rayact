...
interface DevClientOptions {
  ...
  global?: RayactGlobal;
  /**
   * Native hosts own bundle loading + HMR themselves, but still need the
   * `/rayact/debugger` uplink for console forwarding (→ dev-server terminal)
   * and the Chrome DevTools (CDP) channel. In this mode connect() opens only
   * the debugger socket and skips the JS-side HMR/poll bundle-reload paths so
   * they never fight the native host's own reload.
   */
...
export function createDevClient(options: DevClientOptions): RayactDevClient {
  ...
  const connectDebugger = () => {
    ...
    debuggerSocket.onmessage = event => {
      ...
      try {
        message = JSON.parse(event.data) as DevMessage;
      } catch {
        return;
      }
    };
  ...
  const connect = () => {
    if (options.debuggerOnly) {
      // Native host: console + CDP uplink only. No manifest fetch / HMR / poll —
      // the native runtime owns bundle loading and reloads.
      connectDebugger();
      return;
    }
...
export function installConsoleForwarding(client: RayactDevClient, globalObject: RayactGlobal = globalThis as RayactGlobal): void {
  const consoleObject = globalObject.console as (Console & { __rayactConsoleForwarding?: boolean }) | undefined;
  // Guard on the console object itself, not the global: native hosts can share
  // one console across the launcher + project contexts, and a per-global guard
  // would wrap it twice (each forwarded log then appears twice in the terminal).
  if (!consoleObject || consoleObject.__rayactConsoleForwarding) return;

  consoleObject.__rayactConsoleForwarding = true;
