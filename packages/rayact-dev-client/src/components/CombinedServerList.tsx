...
import type { RequiredModule } from '../devServerUrl.js';
...
export function CombinedServerList(props: Props) {
  ...
  const merged: MergedEntry[] = useMemo(() => {
    ...
    for (const s of discoveredServers) {
      ...
      const missingNames = s.missingModules
        ?.map(module => module.jsPackage || module.name)
        .filter(Boolean);
      out.push({
        key,
        url: s.url,
        ...
        compatible,
        dotColor: compatible ? online : mismatch,
        statusText: compatible
          ? undefined
          : `Incompatible — missing ${missingNames?.length ? missingNames.join(', ') : 'required native modules'}`,
        statusColor: compatible ? undefined : mismatch,
        saved: !!saved,
        savedUrl: saved?.url,
      });
    }
    ...
    for (const e of recentEntries) {
      ...
      out.push({
        ...
        statusText: st === 'incompatible' ? 'Incompatible — this client is missing required native modules' : undefined,
        statusColor: st === 'incompatible' ? mismatch : undefined,
        saved: true,
        savedUrl: e.url,
      });
    }
