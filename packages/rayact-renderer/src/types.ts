...
export interface AccessibilityAction {
  name: 'activate' | 'escape' | 'magicTap' | 'increment' | 'decrement' | string;
  label?: string;
}

export interface BaseProps {
  ref?: React.Ref<any>;
  /** Native node identifier. Reserved `__rayact_devtools_*` ids are excluded from inspection. */
  id?: string;
...
export interface ScrollViewProps extends BaseProps {
  ...
  /** Keep a focused TextInput/IME active while tapping ScrollView children. */
  keyboardShouldPersistTaps?: 'always' | 'handled' | 'never';
...
export interface MaterialComponentProps extends BaseProps {
  ...
  wavy?: boolean;
  open?: boolean;
  /** Called when a modal surface requests dismissal (scrim tap or sheet swipe). */
  onRequestClose?: () => void;
  layout?: 'row' | 'column';
  progress?: number;
