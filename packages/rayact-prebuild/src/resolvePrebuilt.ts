...
export function resolvePrebuiltAndroidDir(projectRoot: string, arch: 'arm64' | 'x64' = 'arm64'): string | null {
  return resolvePackageDir(projectRoot, PREBUILT_PACKAGES[arch === 'x64' ? 'android-x64' : 'android-arm64']);
}

export function resolvePrebuiltWebDir(projectRoot: string): string | null {
  return resolvePackageDir(projectRoot, PREBUILT_PACKAGES['web-wasm']);
}
