...
type StackNavigationConfig = {
  ...
  lazyScreens?: boolean;
  /** Constrain scene layers to this navigator's parent instead of the app surface. */
  embedded?: boolean;
};
...
function renderSize(): LayoutSize {
  // Mobile hosts publish the surface size (dp) on every consumed resize —
  // authoritative across rotations, ahead of the render-size getters.
  const published = (globalThis as {
    __rayactWindowDimensions?: { width?: number; height?: number };
  }).__rayactWindowDimensions;
  if (
    published &&
    typeof published.width === 'number' &&
    typeof published.height === 'number' &&
    published.width > 0 &&
    published.height > 0
  ) {
...
// Rotation/resize: scenes are hard-sized to the window (absolute fill with an
// explicit width/height), so Yoga cannot reflow them when the root bounds
// change — their fixed size means onLayout never re-fires either. Native
// invokes __rayactOnDimensionsChange after publishing the new window
// dimensions; every mounted scene re-reads renderSize() off that signal.
const dimensionsListeners = new Set<() => void>();
let dimensionsHookInstalled = false;

function ensureDimensionsHook(): void {
  if (dimensionsHookInstalled) return;
  dimensionsHookInstalled = true;
  const host = globalThis as typeof globalThis & {
    __rayactOnDimensionsChange?: () => void;
  };
...
function useWindowSizeVersion(): number {
  const [version, bump] = React.useReducer((x: number) => x + 1, 0);
  React.useEffect(() => {
    ensureDimensionsHook();
    dimensionsListeners.add(bump);
    return () => {
      dimensionsListeners.delete(bump);
    };
  }, []);
  return version;
}
...
function CachedSceneLayer({
  ...
  useWindowSizeVersion(); // re-render on rotation so sceneSize tracks the window
...
function SceneView({
  ...
  const layoutRef = React.useRef<LayoutSize>(embedded ? { width: 0, height: 0 } : renderSize());
  ...
  const windowSizeVersion = useWindowSizeVersion();
  React.useEffect(() => {
    if (embedded || windowSizeVersion === 0) return;
    const next = renderSize();
    if (
      next.width > 0 &&
      next.height > 0 &&
      (next.width !== layoutRef.current.width ||
        next.height !== layoutRef.current.height)
    ) {
  ...
  }, [windowSizeVersion, embedded, applyStyle]);
  ...
  return (
    <ViewTag
      ...
      style={[
        fill,
        {
          ...(embedded ? {} : { width: sceneSize.width, height: sceneSize.height }),
          backgroundColor: bgColor,
    ...
    >
      {renderContent && contentReady ? (
        <View style={embedded ? { flexGrow: 1 } : { width: sceneSize.width, height: sceneSize.height }}>
          {descriptor.render()}
        </View>
      ) : null}
    </ViewTag>
...
function StackNavigator({
  ...
  // Embedded navigators live inside bounded UI such as the draggable developer
  // tools window. Render only the focused descriptor in normal flow: the full
  // stack scene shell is surface-sized and intentionally used only by app-level
  // navigation. Routing, back behavior, and route lifecycle still come from the
  // same Rayact/React Navigation builder.
  if (embedded) {
    return (
      <NavigationContent>
        <DefaultBackHandler navigation={navigation} state={state} />
        <View style={{ flexGrow: 1, backgroundColor: bgColor }}>
          {focusedDescriptor?.render()}
        </View>
      </NavigationContent>
    );
  }
