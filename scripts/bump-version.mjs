...
// All package.json: version + internal dep ranges. Only scan dirs that exist
// (a packages-only repo has no apps/ or test-projects/).
const pkgRoots = ['packages', 'apps', 'test-projects', 'package.json'].filter((d) => fs.existsSync(path.join(ROOT, d)));
for (const f of execSync(`find ${pkgRoots.join(' ')} -name package.json -maxdepth 3`, { cwd: ROOT, encoding: 'utf8' }).trim().split('\n')) {
  edit(f, (raw) => {
