...
export function resolveRayactSubsystem(root: string, sub: string, srcFile: string, distFile: string): string {
  ...
  const trySrc = (base: string): string | null => {
    const p = path.join(base, 'src', sub, srcFile);
    attempted.push(p);
    return fs.existsSync(p) ? normalizePath(p) : null;
  };
  ...
  try {
    const rayactRoot = path.dirname(req.resolve('rayact/package.json'));
    const linkedSrc = trySrc(rayactRoot);
    if (linkedSrc) return linkedSrc;
    const dist = path.join(rayactRoot, 'dist', distFile);
    attempted.push(dist);
    if (fs.existsSync(dist)) return normalizePath(dist);
  } catch {
    // rayact not installed — continue to the parent-monorepo fallback.
  }
